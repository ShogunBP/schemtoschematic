...
(function() {
	...
	nbt.Reader = function(buffer) {
		...
		this[nbt.tagTypes.byteArray] = function() {
			...
			if (length < 0) throw new Error("Invalid byteArray length: " + length);
			
			// Alocação mais segura
			try {
				const bytes = Buffer.allocUnsafe(length);
				for (let i = 0; i < length; i++) {
					bytes[i] = this.byte() & 0xFF;
				}
				return bytes;
			} catch (e) {
				throw new Error(`Failed to allocate ${length} bytes: ${e.message}`);
			}
		};
		/**
